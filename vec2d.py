...
class Vec2d(object):
    ...
    def get_dist_sqrd_toroidal(self, other, wrap_x, wrap_y):
        """ Distance squared, wrapped in a toroid. """
        diff_x = abs(self.x - other[0])
        diff_y = abs(self.y - other[1])
        if diff_x > wrap_x/2.0:
            diff_x = wrap_x - diff_x
        if diff_y > wrap_y/2.0:
            diff_y = wrap_y - diff_y
        return diff_x**2 + diff_y**2
    
    def toroidal_sub(self, other, wrap_x, wrap_y):
        """ Vector substraction in a toroid. """
        diff_x = self.x - other[0]
        diff_y = self.y - other[1]
        abs_diff_x = abs(diff_x)
        abs_diff_y = abs(diff_y)
        
        if abs_diff_x > wrap_x/2.0:
            #diff_x = (wrap_x - abs_diff_x)
            # copy reversed sign of original difference, for the real distance
            diff_x = math.copysign((wrap_x - abs_diff_x), -diff_x)
        if abs_diff_y > wrap_y/2.0:
            #diff_y = (wrap_y - abs_diff_y)
            diff_y = math.copysign((wrap_y - abs_diff_y), -diff_y)
            
        return Vec2d(diff_x, diff_y)
